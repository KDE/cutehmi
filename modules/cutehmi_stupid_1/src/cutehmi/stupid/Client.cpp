...
#include "../../../include/cutehmi/stupid/internal/functions.hpp"
...
{
	if (!isDisconnected()) {
		CUTEHMI_LOG_WARNING("Trying to connect, while not disconnected.");
		return;
	}
...
{
	if (isDisconnected()) {
		CUTEHMI_LOG_WARNING("Already disconnected.");
		return;
	} else if (!isConnected())
		CUTEHMI_LOG_WARNING("Forcing disconnect, while connection has not been fully established (connection status is '" << static_cast<int>(m->asyncConnector->status()) << "'.");
...
void Client::readAll(const QAtomicInt & run)
{
	...
	if (!isConnected()) {
		CUTEHMI_LOG_WARNING("Read attempt without established connection (connection status: '" << static_cast<int>(m->asyncConnector->status()) << "').");
		return;
	}
...
void Client::asyncConnect(QObject * connector)
{
	if (m->asyncConnector != connector) {
		CUTEHMI_LOG_WARNING("Received orphaned signal. Connection has been aborted.");
		return;
	}
	...
	CUTEHMI_LOG_DEBUG("Connection has been established.");
	m->connected = true;
	emit connected();
}
