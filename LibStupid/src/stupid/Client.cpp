...
QString Client::Error::str() const
{
	switch (code()) {
		case Error::UNABLE_TO_CONNECT:
			return tr("Unable to open connection with database.");
		default:
			return base::Error::str();
	}
}

Client::Client(const QString & connectionName, QObject * parent):
	QObject(parent),
...
void Client::connect()
{
	...
	if (db.open()) {
		qDebug("[TODO provide AppLite with a UI for signaling errors] Connected to database.");
		emit connected();
	} else {
		qDebug("[TODO provide AppLite with a UI for signaling errors] Could not connect to database.");
...
void Client::enumerateDevices()
{
	...
	query.exec("SELECT w1_device.w1_id "
			   "FROM ds18b20 INNER JOIN w1_device ON ds18b20.w1_device_id = w1_device.id");
	while (query.next())
		m_ds18b20.insert(query.value(0).toString(), QVariant::fromValue(new DS18B20));
}
